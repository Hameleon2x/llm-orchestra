...
- Config sections are now strict about typos: an unknown key in the catalog top level, a model, provider or policy record, in generation params, or in run options (`RunOptions`) throws `LlmConfigException` while the catalog is built. Drop any stray keys the library used to ignore silently, or move arbitrary application data into `meta`.
