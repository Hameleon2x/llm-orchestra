...
- Секции конфига теперь строгие к опечаткам: неизвестный ключ в записи модели, провайдера или политики, в параметрах генерации или в опциях прогона (`RunOptions`) бросает `LlmConfigException` при сборке каталога. Уберите «лишние» ключи, которые библиотека раньше молча игнорировала, или перенесите произвольные данные в `meta`.
