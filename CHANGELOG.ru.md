...
- **Ошибки настройки транспорта** (битый адрес, неизвестный протокол, проблема с сертификатом — cURL 1/3/60/77) получают категорию `config`: повторять их и перебирать модели бессмысленно.
