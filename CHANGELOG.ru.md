...
- **Изоляция прикладных колбэков.** Исключение из приёмника событий (`$emit`) и из наблюдателя попыток (`withObserver()`) больше не обрывает прогон: оно пишется в лог (`LLM event sink failed`, `LLM attempt observer failed`). Исключение из самого инструмента закрывает его вызов ошибкой — модель увидит её и продолжит, а подробности уйдут в лог (`LLM tool threw an exception`). Текст исключения модели по умолчанию не показывается: `Agent\Dto\Config::$exposeToolExceptions` включает это явно, с обрезкой до 300 символов.
...
- **Один уровень повторов вместо двух.** Транспорт больше не крутит свой цикл: повторы считает политика модели (`retries`, `delay`, `backoff`, `maxDelay`, `perCategory`, `retryOn`, `stopOn`, `maxWaitSeconds`). Время ожидания при сбое стало предсказуемым; уже идущий запрос ограничивает `timeout` провайдера или модели.
...
- **`Config::$maxTurns` по умолчанию `40`** (при `maxToolCalls = 30`). Оборотов должно хватать на все разрешённые вызовы инструментов, иначе прогон упирается в лимит оборотов и отдаёт служебную заглушку вместо итогового ответа модели.
