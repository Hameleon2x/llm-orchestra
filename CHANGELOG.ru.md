...
- **Срок прогона удерживается внутри оборота.** `Config::$deadlineSeconds` проецируется на каждый вызов исполнителя через `Orchestra::withTotalWaitSeconds()`, поэтому повторы и переключения не уводят прогон за дедлайн.
...
- **`defaultDeadlineSeconds` каталога** — срок прогона по умолчанию: его берёт цикл, когда прогон не задал свой. Срок описывает установку (веб-воркер против консольной команды), а не задачу, поэтому повторять его в каждом вызывающем сервисе не нужно.
- **Тексты, уходящие модели, настраиваются:** `toolLimitReachedText`, `toolFailedText`, `toolFailedPrefix`, `encodeFailedText` и `firstUseResultKey` в `Agent\Dto\Config` — раньше эти строки были зашиты в `Runner`.
...
- **`Agent\Dto\Config::$maxSwitches`** — предел переключений на этот прогон (раньше задавался только каталогом).
...
- **Пояснение по тулзе (`firstUseHint`) подмешивается в её РЕЗУЛЬТАТ при первом вызове, а не в системный промт.** Раньше `Agent\SystemPromptComposer` дописывал в system-промт блок пояснений по всем уже вызванным тулзам и пересобирал промт каждый оборот — мутирующий системный префикс сбрасывал prompt-кеш провайдера (OpenAI/Grok/DeepSeek/Gemini и др. кешируют по префиксу) на всей истории запроса. Теперь системный промт неизменен между оборотами, а `Runner` при ПЕРВОМ вызове тулзы в диалоге кладёт её пояснение прямо в JSON-результат под ключом `firstUseHintKey()`. История append-only, префикс запроса стабилен — кеш переиспользуется.
  ...
  - Пояснение подмешивается только если непустое; результат-список при этом убирается под `Config::$firstUseResultKey` (по умолчанию `result`), потому что ключ в список не добавить; при нескольких одноимённых вызовах в одном ходе — только в первый; на возобновлении/повторе прогона не дублируется (первенство — по самому раннему вхождению имени тулзы в историю).
