...
- **HTTP 402 (out of credits / billing limit) maps to `model_unavailable`** — a fallbackable, non-retryable category: a provider running out of credits hands the work to the next model in the chain instead of failing the whole call.
- **A typo in a config value no longer passes silently:** `then`, the categories in `retryOn`, `stopOn` and `perCategory`, and the names in `unsupported` are checked against the allowed sets while the catalog is built. Otherwise `'Stop'` instead of `'stop'` would quietly permit the very model switching it is meant to forbid, and a non-empty `retryOn` without a single real category would disable retries entirely. On top of that, an unknown key in any config section — the catalog top level, a model, provider or policy record, generation params, or run options — is now rejected the same way `RunOptions` already did (one shared check): a mistyped `maxTokns` or `retrie` no longer vanishes without a trace.
...
- **The escalation decision follows the model that failed, not the one the run started on.** When a fallback model exhausts its retries, whether to escalate further is decided by that model's policy (`then`, `stopOn`) — the same policy its retries used. Previously this one decision always used the starting model's policy, so a `then: stop` on a fallback model was ignored.
