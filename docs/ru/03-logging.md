...
Агентский цикл пишет ещё четыре записи — им нужен логгер во втором аргументе конструктора: `new Runner($orchestra, $logger)`.
...
- `error` **`LLM run aborted by application code`** — упал системный промт или реестр инструментов; прогон возвращает ошибку категории `config` с сохранённой историей. Контекст: `stage`, `message`, `exception`.
- `warning` **`LLM event sink failed`** — приёмник событий бросил исключение; прогон продолжается. Контекст: `event`, `message`.
- `warning` **`LLM first-use hint failed`** — не удалось получить пояснение к первому вызову инструмента; результат инструмента при этом сохраняется. Контекст: `tool`, `message`.
