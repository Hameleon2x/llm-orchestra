...
- **`Config::$maxToolCalls`** — сколько инструментов можно исполнить. При исчерпании делается ещё один запрос без инструментов, чтобы модель подвела итог по собранным данным; результат помечается `Finish::TOOL_LIMIT`, а если и этот запрос не удался — ошибкой с категорией.
- **`Config::$deadlineSeconds`** — предельная длительность прогона. Проверяется перед каждым оборотом, а остаток срока уходит исполнителю как потолок ожидания на вызов, поэтому повторы внутри оборота тоже им ограничены. При истечении возвращается ошибка категории `deadline` вместе с полной историей.
