...
Исключение из приёмника прогон не обрывает: цикл перехватывает его и пишет в лог `LLM event sink failed`. К моменту события инструменты уже отработали с побочными эффектами, и терять из-за упавшей записи в базу всю историю нельзя.
