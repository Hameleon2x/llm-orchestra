...
Цепочку запасных моделей, число переключений и политику ошибок задаёт каталог — это свойство установки, а не прогона. Прогону, которому нужна другая цепочка, передайте свой `Orchestra::withFallback()`/`withPolicy()` (см. [02-catalog-and-fallback.md](02-catalog-and-fallback.md)).
