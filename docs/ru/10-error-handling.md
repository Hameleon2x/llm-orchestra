...
Сбой вызова модели наружу исключением не выходит: и `Orchestra`, и `Runner` возвращают результат, внутри которого лежит ошибка. Исключение из вашего кода наружу тоже не выходит: сбой инструмента закрывает его вызов ошибкой и прогон продолжается, сбой системного промта или реестра инструментов возвращается результатом с категорией `config`, а сбой приёмника событий только пишется в лог. Успех — это отсутствие ошибки:
...
- `NETWORK` — обрыв соединения, DNS: ошибки cURL, кроме таймаута и ошибок настройки (см. `CONFIG`).
...
- `MODEL_UNAVAILABLE` — нет такой модели, снята с обслуживания, перегружена (404); а также кончились кредиты / лимит биллинга (402).
...
- `CONFIG` — ошибка каталога, битый адрес или сертификат провайдера, сбой прикладного кода в агентском цикле.
...
HTTP-статус задаёт категорию не жёстко: у ответов 4xx её уточняет текст ошибки — переполненный контекст и блокировка модерацией приезжают под тем же HTTP 400. Ответы 5xx и 429 текстом не переопределяются. Слово «таймаут» не переопределяет ни один ответ с кодом ошибки: статус говорит о запросе больше, чем формулировка. Редирект (3xx) в ответ на POST — всегда `config`: так выглядит неверно указанный адрес провайдера.
...
`LlmConfigException` возникает при сборке каталога и при прямом обращении к неизвестной модели через `Registry` — то есть тогда, когда ошибку ещё можно исправить в конфиге. Через `Orchestra` неизвестный ключ исключением не становится: он подменяется моделью каталога по умолчанию с предупреждением в лог, а если нет и её — возвращается ошибка категории `config`. Его имеет смысл поймать при старте приложения, чтобы показать понятное сообщение.
