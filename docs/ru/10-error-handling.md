...
Сбой вызова модели наружу исключением не выходит: и `Orchestra`, и `Runner` возвращают результат, внутри которого лежит ошибка. Исключение из вашего кода — системного промта, реестра инструментов, приёмника событий — цикл не перехватывает, это ваш поток управления; исключение из самого инструмента перехватывается и закрывает вызов ошибкой. Успех — это отсутствие ошибки:
