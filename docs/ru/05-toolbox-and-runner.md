...
| `$systemPromptFn` | Вызывается каждый ход с текущей историей. Верни базовый system-промт — `SystemPromptComposer` его дополнит.         |
...
## Как растёт system-промт: `SystemPromptComposer`

Каждый ход runner вызывает `$systemPromptFn($messages)`, получает базовый промт и отдаёт его `Agent\SystemPromptComposer::compose()`. Composer:

1. Сканирует `$messages` на ассистентские ходы с `tool_calls`.
2. Собирает уникальные имена использованных тулз (отсортированные по алфавиту).
3. Зовёт `$toolbox->systemPromptAddition($name)`, который делегирует в `ToolInterface::appendToSystemPromptAfterUse()`.
4. Дописывает непустые заметки под заголовком `"Дополнительные пояснения по уже использованным инструментам:"` (текст по-русски — меняй `SystemPromptComposer::TOOL_NOTES_HEADER`, если форкаешь пакет).

В итоге: тулзы, которые ни разу не вызывали, ничего не добавляют; тулзы, которые уже вызвали, со следующего хода дописывают свои заметки по схеме вывода.
