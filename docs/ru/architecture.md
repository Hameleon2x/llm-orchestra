...
- **`Http\CurlChatClient`** — отправка HTTP и транспортные ошибки. Успешное тело отдаёт провайдеру как есть; разбирает только ответ с кодом ошибки, чтобы приложить к сбою категорию.
