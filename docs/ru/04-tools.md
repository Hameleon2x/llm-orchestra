...
- **`firstUseHint(): string`** — пояснение, как читать **ответ** инструмента. Подмешивается в его результат при первом вызове в диалоге. По умолчанию пусто. Результат-объект получает пояснение соседним ключом, а результат-список на первом вызове убирается под ключ `RunOptions::$firstUseResultKey` (по умолчанию `result`) — рядом с пояснением: добавить ключ в список нельзя, а терять пояснение не хочется.
...
Если ваши инструменты бросают осмысленные для модели исключения, включите `$options->exposeToolExceptions = true`: сообщение придёт модели одной строкой, обрезанное до 300 символов.
...
От одного класса ошибок цикл защищает сам: если модель прислала аргументы с протёкшей разметкой вызова (`<parameter name="...">` внутри значения), инструмент не будет исполнен — модель получит ошибку и переотправит вызов. Это `RunOptions::$toolArgsGuard`, он включён по умолчанию.
