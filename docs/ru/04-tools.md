...
Три метода — и модель уже умеет узнавать время. `AbstractTool` закрывает остальные методы контракта разумными значениями по умолчанию.
...
- **`firstUseHint(): string`** — пояснение, как читать **ответ** инструмента. Подмешивается в его результат при первом вызове в диалоге. По умолчанию пусто.
...
Если исключение всё же вылетит из `execute()`, цикл его перехватит и закроет вызов ошибкой с текстом исключения — прогон не оборвётся, но сообщение будет техническим, не рассчитанным на модель.
