...
Каталог создаёт провайдера как `new $class($definition, $logger)` — конструктор с такой сигнатурой обязателен. Проще всего наследовать `BaseProvider`: он даёт конструктор, доступ к настройкам, HTTP-клиент и применение карты `capture`.
